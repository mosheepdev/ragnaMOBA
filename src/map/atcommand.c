		...
		if (strcmpi(w1, "import") == 0) {
			...
			if (msg_number >= 0 && msg_number < MAX_MSG) {
				if (atcommand->msg_table[msg_number] != NULL) {
					if (!allow_override) {
						ShowError("Mensagem duplicada: ID '%d' ja foi usada para '%s'. A mensagem '%s' sera ignorada.\n",
...
#define GET_VALUE(p,num) do { \
	...
	if (type >= MIN_PACKET_DB && type <= MAX_PACKET_DB) {
		...
		if (len) {
			// show packet length
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(904), type, clif->packet(type)->len); // Packet 0x%x length: %d
			clif->message(fd, atcmd_output);
			return true;
		}
		...
		if (len == 0) {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(905), type); // Unknown packet: 0x%x
			clif->message(fd, atcmd_output);
			return false;
		} else if (len == -1) {
			// dynamic packet
...
ACMD(mapmove) {
	...
	if( pc_isdead(sd) ) {
		clif->message(fd, msg_txt(864)); // "Você não pode usar este comando caso esteja morto."
		return false;
	}
...
ACMD(jumpto) {
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(4), pl_sd->status.name); // Jumped to %s
	clif->message(fd, atcmd_output);
...
ACMD(jump)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(5), sd->bl.x, sd->bl.y); // Jumped to %d %d
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(whogm)
{
	...
	for (pl_sd = (TBL_PC*)mapit->first(iter); mapit->exists(iter); pl_sd = (TBL_PC*)mapit->next(iter)) {
		...
		if (pl_level > level) {
			if (pc_isinvisible(pl_sd))
				continue;
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(913), pl_sd->status.name); // Name: %s (GM)
			clif->message(fd, atcmd_output);
			count++;
			continue;
		}

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(914), // Name: %s (GM:%d) | Location: %s %d %d
				pl_sd->status.name, pl_level,
				mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
		clif->message(fd, atcmd_output);

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(915), // BLvl: %d | Job: %s (Lvl: %d)
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(916), // Party: '%s' | Guild: '%s'
	...
	if (count == 0)
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(152), count); // %d GMs found.
		clif->message(fd, atcmd_output);
	}
...
ACMD(speed)
{
	...
	if (!*message || sscanf(message, "%12d", &speed) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(918), MIN_WALK_SPEED, MAX_WALK_SPEED); // Please enter a speed value (usage: @speed <%d-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(help) {
	...
	if (!atcommand->can_use2(sd, command_name, COMMAND_ATCOMMAND)) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(153), message); // "%s is Unknown Command"
		clif->message(fd, atcmd_output);
	...
	if ( !tinfo || tinfo->help == NULL ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(988), atcommand->at_symbol, command_name); // There is no help for %c%s.
		clif->message(fd, atcmd_output);
		atcommand->get_suggestions(sd, command_name, true);
		return false;
	}

	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(989), atcommand->at_symbol, command_name); // Help for command %c%s:
	clif->message(fd, atcmd_output);
...
ACMD(model)
{
	...
	if (!*message || sscanf(message, "%12d %12d %12d", &hair_style, &hair_color, &cloth_color) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(991), // Please enter at least one value (usage: @model <hair ID: %d-%d> <hair color: %d-%d> <clothes color: %d-%d>).
...
ACMD(dye)
{
	...
	if (!*message || sscanf(message, "%12d", &cloth_color) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(992), MIN_CLOTH_COLOR, MAX_CLOTH_COLOR); // Please enter a clothes color (usage: @dye/@ccolor <clothes color: %d-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(hair_style)
{
	...
	if (!*message || sscanf(message, "%12d", &hair_style) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(993), MIN_HAIR_STYLE, MAX_HAIR_STYLE); // Please enter a hair style (usage: @hairstyle/@hstyle <hair ID: %d-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(hair_color)
{
	...
	if (!*message || sscanf(message, "%12d", &hair_color) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(994), MIN_HAIR_COLOR, MAX_HAIR_COLOR); // Please enter a hair color (usage: @haircolor/@hcolor <hair color: %d-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(go) {
	...
	const struct {
		...
		{ MAP_DICASTES,    198, 187, 3 }, //  0 = El Dicastes	// [aleDsz] Apenas um mapa no @go
	};
	...
	if (!*message || sscanf(message, "%11s", map_name) < 1) {
		...
		// Deixar uma mensagem fixa para utilizar o @go
		text = "Para se telestranportar para a cidade, digite @go 0.";
...
ACMD(monster)
{
	...
	if (count != 0)
		if (number == count)
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(240), count); // %d monster(s) summoned!
			clif->message(fd, atcmd_output);
		}
		else {
			clif->message(fd, msg_txt(40)); // Invalid monster ID or name.
...
ACMD(refine)
{
	...
	if (!*message || sscanf(message, "%12d %12d", &position, &refine) < 2) {
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(997), EQP_HEAD_LOW); // %d: Lower Headgear
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(998), EQP_HAND_R); // %d: Right Hand
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(999), EQP_GARMENT); // %d: Garment
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1000), EQP_ACC_L); // %d: Left Accessory
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1001), EQP_ARMOR); // %d: Body Armor
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1002), EQP_HAND_L); // %d: Left Hand
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1003), EQP_SHOES); // %d: Shoes
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1004), EQP_ACC_R); // %d: Right Accessory
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1005), EQP_HEAD_TOP); // %d: Top Headgear
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1006), EQP_HEAD_MID); // %d: Mid Headgear
		clif->message(fd, atcmd_output);
		return false;
	}
	...
	if (count == 0)
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(168), count); // %d items have been refined.
		clif->message(fd, atcmd_output);
	}
...
ACMD(produce)
{
	...
	if (itemdb->isequip2(item_data)) {
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(169), item_id, item_data->name); // The item (%d: '%s') is not equipable.
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(memo)
{
	...
	if (!*message || sscanf(message, "%d", &position) < 1)
	{
		...
		for( i = 0; i < MAX_MEMOPOINTS; i++ )
		{
			if( sd->status.memo_point[i].map )
				...
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(171), i); // %d - void
	...
	{
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1008), 0, MAX_MEMOPOINTS-1); // Please enter a valid position (usage: @memo <memo_position:%d-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(recall) {
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(46), pl_sd->status.name); // %s recalled!
	clif->message(fd, atcmd_output);
...
ACMD(idsearch)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(77), item_name); // Search results for '%s' (name: id):
	clif->message(fd, atcmd_output);
	match = itemdb->search_name_array(item_array, MAX_SEARCH, item_name, 0);
	if (match > MAX_SEARCH) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(269), MAX_SEARCH, match);
		clif->message(fd, atcmd_output);
		match = MAX_SEARCH;
	}
	for(i = 0; i < match; i++) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(78), item_array[i]->jname, item_array[i]->nameid); // %s: %d
		clif->message(fd, atcmd_output);
	}
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(79), match); // %d results found.
	clif->message(fd, atcmd_output);
...
ACMD(recallall)
{
	...
	if (count) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1033), count); // Because you are not authorized to warp from some maps, %d player(s) have not been recalled.
		clif->message(fd, atcmd_output);
	}
...
ACMD(guildrecall)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(93), g->name); // All online characters of the %s guild have been recalled to your position.
	clif->message(fd, atcmd_output);
	if (count) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1033), count); // Because you are not authorized to warp from some maps, %d player(s) have not been recalled.
		clif->message(fd, atcmd_output);
...
ACMD(partyrecall)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(95), p->party.name); // All online characters of the %s party have been recalled to your position.
	clif->message(fd, atcmd_output);
	if (count) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1033), count); // Because you are not authorized to warp from some maps, %d player(s) have not been recalled.
		clif->message(fd, atcmd_output);
...
ACMD(mapinfo) {
	...
	if (map->list[m_id].flag.nosave) {
		if (!map->list[m_id].save.map)
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1069), mapindex_id2name(map->list[m_id].save.map)); // No Save, Save Point: %s,Random
			clif->message(fd, atcmd_output);
		} else {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1070), // No Save, Save Point: %s,%d,%d
	...
	switch (list) {
		...
		case 1:
			...
			{
				if (pl_sd->mapindex == m_index) {
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1099), // Player '%s' (session #%d) | Location: %d,%d
		...
		case 2:
			...
			for (i = 0; i < map->list[m_id].npc_num;) {
				...
				if(strcmp(nd->name,nd->exname) == 0)
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1111), // NPC %d: %s | Direction: %s | Sprite: %d | Location: %d %d
						++i, nd->name, direction, nd->class_, nd->bl.x, nd->bl.y);
				else
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1112), // NPC %d: %s::%s | Direction: %s | Sprite: %d | Location: %d %d
		...
		case 3:
			...
			for (pl_sd = (TBL_PC*)mapit->first(iter); mapit->exists(iter); pl_sd = (TBL_PC*)mapit->next(iter))
			{
				if ((cd = (struct chat_data*)map->id2bl(pl_sd->chatID)) != NULL &&
						...
						cd->usersd[0] == pl_sd)
				{
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1114), // Chat: %s | Player: %s | Location: %d %d
							cd->title, pl_sd->status.name, cd->bl.x, cd->bl.y);
					clif->message(fd, atcmd_output);
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1115), //    Users: %d/%d | Password: %s | Public: %s
...
ACMD(mount_peco)
{
	...
	if ((sd->class_&MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT) {
		if (!pc->checkskill(sd,RK_DRAGONTRAINING)) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(213), skill->get_desc(RK_DRAGONTRAINING)); // You need %s to mount!
			clif->message(fd, atcmd_output);
			return false;
		}
	...
	if ((sd->class_&MAPID_THIRDMASK) == MAPID_RANGER) {
		if (!pc->checkskill(sd,RA_WUGRIDER)) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(213), skill->get_desc(RA_WUGRIDER)); // You need %s to mount!
			clif->message(fd, atcmd_output);
			return false;
		}
	...
	if (sd->class_&MAPID_SWORDMAN && sd->class_&JOBL_2) {
		if (!pc_isridingpeco(sd)) { // if actually no peco
			if (!pc->checkskill(sd, KN_RIDING)) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(213), skill->get_desc(KN_RIDING)); // You need %s to mount!
				clif->message(fd, atcmd_output);
				return false;
			}
...
ACMD(guildspy) {
	...
	if ((g = guild->searchname(guild_name)) != NULL || // name first to avoid error when name begin with a number
		...
		if (sd->guildspy == g->guild_id) {
			sd->guildspy = 0;
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(103), g->name); // No longer spying on the %s guild.
			clif->message(fd, atcmd_output);
		} else {
			sd->guildspy = g->guild_id;
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(104), g->name); // Spying on the %s guild.
			clif->message(fd, atcmd_output);
		}
	} else {
...
ACMD(partyspy) {
	...
	if ((p = party->searchname(party_name)) != NULL || // name first to avoid error when name begin with a number
		...
		if (sd->partyspy == p->party.party_id) {
			sd->partyspy = 0;
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(105), p->party.name); // No longer spying on the %s party.
			clif->message(fd, atcmd_output);
		} else {
			sd->partyspy = p->party.party_id;
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(106), p->party.name); // Spying on the %s party.
			clif->message(fd, atcmd_output);
		}
	} else {
...
char* txt_time(unsigned int duration) {
	...
	if (days == 1)
		tlen += sprintf(tlen + temp1, msg_txt(219), days); // %d dia
	else if (days > 1)
		tlen += sprintf(tlen + temp1, msg_txt(220), days); // %d dias
	if (hours == 1)
		tlen += sprintf(tlen + temp1, msg_txt(221), hours); // %d hora
	else if (hours > 1)
		tlen += sprintf(tlen + temp1, msg_txt(222), hours); // %d horas
	if (minutes < 2)
		tlen += sprintf(tlen + temp1, msg_txt(223), minutes); // %d minuto
	else
		tlen += sprintf(tlen + temp1, msg_txt(224), minutes); // %d minutos
	if (seconds == 1)
		sprintf(tlen + temp1, msg_txt(225), seconds); // e %d segundo
	else if (seconds > 1)
		sprintf(tlen + temp1, msg_txt(226), seconds); // e %d segundos
...
ACMD(jailfor) {
	...
	if (pl_sd->sc.data[SC_JAILED] && pl_sd->sc.data[SC_JAILED]->val1 != INT_MAX) {
		...
		if (jailtime <= 0) {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(402),msg_txt(1137),year,month,day,hour,minute); //%s in jail for %d years, %d months, %d days, %d hours and %d minutes
			clif->message(pl_sd->fd, atcmd_output);
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(402),msg_txt(1138),year,month,day,hour,minute); //This player is now in jail for %d years, %d months, %d days, %d hours and %d minutes
			clif->message(fd, atcmd_output);
		}
	} else if (jailtime < 0) {
...
ACMD(jailtime)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(402),msg_txt(1142),year,month,day,hour,minute); // You will remain in jail for %d years, %d months, %d days, %d hours and %d minutes
...
ACMD(addwarp)
{
	...
	if( m == 0 )
	{
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1157), mapname); // Unknown map '%s'.
		clif->message(fd, atcmd_output);
		return false;
	}
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1158), nd->exname); // New warp NPC '%s' created.
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(skillid) {
	...
	for (data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key)) {
		...
		if (strnicmp(key.str, message, skillen) == 0 || strnicmp(skill->dbs->db[idx].desc, message, skillen) == 0) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1164), DB->data2i(data), skill->dbs->db[idx].desc, key.str); // skill %d: %s (%s)
			clif->message(fd, atcmd_output);
		} else if ( found < MAX_SKILLID_PARTIAL_RESULTS && ( stristr(key.str,message) || stristr(skill->dbs->db[idx].desc,message) ) ) {
			snprintf(partials[found], MAX_SKILLID_PARTIAL_RESULTS_LEN, msg_txt(1164), DB->data2i(data), skill->dbs->db[idx].desc, key.str);
			found++;
		}
	}
	...
	if( found ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1398), found); // -- Displaying first %d partial matches
		clif->message(fd, atcmd_output);
	}
...
ACMD(skilltree) {
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1168), pc->job_name(c), pc->checkskill(pl_sd, NV_BASIC)); // Player is using %s skill tree (%d basic points).
	clif->message(fd, atcmd_output);
	...
	for(j=0;j<MAX_PC_SKILL_REQUIRE;j++)
	{
		...
		{
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1170), ent->need[j].lv, skill->dbs->db[ent->need[j].id].desc); // Player requires level %d of skill %s.
			clif->message(fd, atcmd_output);
			meets = 0;
		}
	}
...
{
	if (pc->divorce(sd) != 0) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1175), sd->status.name); // '%s' is not married.
		clif->message(fd, atcmd_output);
		return false;
	}

	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1176), sd->status.name); // '%s' and his/her partner are now divorced.
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(autolootitem)
{
	...
	switch (action) {
		case 1:
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1192), item_data->name, item_data->jname, item_data->nameid); // Autolooting item: '%s'/'%s' {%d}
			clif->message(fd, atcmd_output);
		...
		case 2:
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1194), item_data->name, item_data->jname, item_data->nameid); // Removed item: '%s'/'%s' {%d} from your autolootitem list.
			clif->message(fd, atcmd_output);
			ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
			if (i == AUTOLOOTITEM_SIZE) {
			...
			break;
		case 3:
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1195), AUTOLOOTITEM_SIZE); // You can have %d items on your autolootitem list.
			clif->message(fd, atcmd_output);
			...
			if (i == AUTOLOOTITEM_SIZE) {
				...
				for(i = 0; i < AUTOLOOTITEM_SIZE; i++)
				{
					...
					if (!(item_data = itemdb->exists(sd->state.autolootid[i]))) {
						ShowDebug("Item nao existente %d na lista do autolootitem (account_id: %d, char_id: %d)", sd->state.autolootid[i], sd->status.account_id, sd->status.char_id);
						continue;
					}
...
ACMD(autoloottype) {
	...
	switch (action) {
		case 1:
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1492), itemdb->typename(type)); // Autolooting item type: '%s'
			clif->message(fd, atcmd_output);
			break;
		case 2:
			if (!(sd->state.autoloottype&(1<<type))) {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1494), itemdb->typename(type)); // Removed item type: '%s' from your autoloottype list.
			clif->message(fd, atcmd_output);
			break;
		case 3:
			clif->message(fd, msg_txt(38)); // Invalid location number, or name.
...
ACMD(reset) {
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(208), sd->status.name); // '%s' skill and stats points reseted!
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(homlevel) {
	...
	if ((htype = homun->class2type(hd->homunculus.class_)) == HT_INVALID) {
		ShowError("atcommand_homlevel: classe homunculus invalida %d (player %s)\n", hd->homunculus.class_,sd->status.name);
		return false;
	}
	...
	switch( htype ) {
		...
		case HT_S:
			...
			break;
		default:
			ShowError("atcommand_homlevel: tipo de homunculus desconhecido '%d'\n",htype);
			return false;
	}
...
ACMD(iteminfo)
{
	...
	if (count > MAX_SEARCH) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(269), MAX_SEARCH, count); // Displaying first %d out of %d matches
		clif->message(fd, atcmd_output);
		count = MAX_SEARCH;
	}
	for (i = 0; i < count; i++) {
		struct item_data *item_data = item_array[i];
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1277), // Item: '%s'/'%s'[%d] (%d) Type: %s | Extra Effect: %s
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1280), item_data->value_buy, item_data->value_sell, item_data->weight/10. ); // NPC Buy:%dz, Sell:%dz | Weight: %.1f
		clif->message(fd, atcmd_output);
		...
		if (item_data->maxchance == -1)
			...
			if( item_data->maxchance )
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1282), (float)item_data->maxchance / 100 ); //  - Maximal monsters drop chance: %02.02f%%
			else
...
ACMD(whodrops)
{
	...
	for (i = 0; i < count; i++) {
		struct item_data *item_data = item_array[i];
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1285), item_data->jname,item_data->slot); // Item: '%s'[%d]
		clif->message(fd, atcmd_output);
		...
		if (item_data->mob[0].chance == 0) {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1287), MAX_SEARCH); //  - Common mobs with highest drop chance (only max %d are listed):
			clif->message(fd, atcmd_output);
...
ACMD(whereis)
{
	...
	if (count > MAX_SEARCH) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(269), MAX_SEARCH, count);
		clif->message(fd, atcmd_output);
		count = MAX_SEARCH;
	}
	for (k = 0; k < count; k++) {
...
ACMD(version) {
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1296), sysinfo->is64bit() ? 64 : 32, sysinfo->platform()); // Hercules %d-bit for %s
	clif->message(fd, atcmd_output);
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1295), sysinfo->vcstype(), sysinfo->vcsrevision_src(), sysinfo->vcsrevision_scripts()); // %s revision '%s' (src) / '%s' (scripts)
	clif->message(fd, atcmd_output);
...
ACMD(rates)
{
	...
	safesnprintf(buf, CHAT_SIZE_MAX, msg_txt(1298), // Experience rates: Base %.2fx / Job %.2fx
			 battle_config.base_exp_rate/100., battle_config.job_exp_rate/100.);
	clif->message(fd, buf);
	safesnprintf(buf, CHAT_SIZE_MAX, msg_txt(1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
	...
	clif->message(fd, buf);
	safesnprintf(buf, CHAT_SIZE_MAX, msg_txt(1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
	...
	clif->message(fd, buf);
	safesnprintf(buf, CHAT_SIZE_MAX, msg_txt(1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
...
ACMD(invite) {
	...
	if (battle_config.duel_only_on_same_map && target_sd->bl.m != sd->bl.m)
	{
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(364), message);
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(auction)
{
	if (!battle_config.feature_auction) {
		clif->messagecolor_self(sd->fd, COLOR_RED, msg_txt(1484));
		return false;
	}
...
{
	pc->resetstate(sd);
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(207), sd->status.name);
	clif->message(fd, atcmd_output);
	return true;
}
...
{
	pc->resetskill(sd, PCRESETSKILL_RESYNC);
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(206), sd->status.name);
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(delitem) {
	...
	// notify target
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(113), total-amount); // %d item(s) removed by a GM.
	clif->message(sd->fd, atcmd_output);
	...
	{
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(115), total-amount, total-amount, total); // %d item(s) removed. Player had only %d on %d items.
		clif->message(fd, atcmd_output);
	}
	else
	{
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(114), total); // %d item(s) removed from the player.
		clif->message(fd, atcmd_output);
	}
	return true;
}
...
void atcommand_commands_sub(struct map_session_data* sd, const int fd, AtCommandType type)
{
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(274), count); // "%d commands found."
	clif->message(fd, atcmd_output);
...
ACMD(set) {
	...
	switch( data->type ) {
		case C_INT:
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1373),reg,data->u.num); // %s value is now :%d
			break;
		case C_STR:
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1374),reg,data->u.str); // %s value is now :%s
			break;
		case C_CONSTSTR:
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1375),reg); // %s is empty
			break;
		default:
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1376),reg,data->type); // %s data type is not supported :%u
			break;
	}
	clif->message(fd, atcmd_output);
...
ACMD(addperm) {
	...
	if (!*message) {
		safesnprintf(atcmd_output, sizeof(atcmd_output),  msg_txt(1378),command); // Usage: %s <permission_name>
		clif->message(fd, atcmd_output);
	...
	if( i == perm_size ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1380),message); // '%s' is not a known permission.
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1379)); // -- Permission List
		for( i = 0; i < perm_size; i++ ) {
			safesnprintf(atcmd_output, sizeof(atcmd_output),"- %s",pcg->permissions[i].name);
	...
	if( add && (sd->extra_temp_permissions&pcg->permissions[i].permission) ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output),  msg_txt(1381),sd->status.name,pcg->permissions[i].name); // User '%s' already possesses the '%s' permission.
		clif->message(fd, atcmd_output);
		return false;
	} else if ( !add && !(sd->extra_temp_permissions&pcg->permissions[i].permission) ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output),  msg_txt(1382),sd->status.name,pcg->permissions[i].name); // User '%s' doesn't possess the '%s' permission.
		clif->message(fd, atcmd_output);
		safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1383),sd->status.name); // -- User '%s' Permissions
		clif->message(fd, atcmd_output);
		for( i = 0; i < perm_size; i++ ) {
	...
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1384),sd->status.name); // User '%s' permissions updated successfully. The changes are temporary.
	clif->message(fd, atcmd_output);
...
#define MC_CART_MDFY(x,idx) do { \
	...
	if (!*message || val < 0 || val > MAX_CARTS) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1390),command,MAX_CARTS); // Unknown Cart (usage: %s <0-%d>).
		clif->message(fd, atcmd_output);
		return false;
	}
...
ACMD(join)
{
	...
	if (!*message || sscanf(message, "%19s %19s", name, pass) < 1) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1399),command); // Unknown Channel (usage: %s <#channel_name>)
		clif->message(fd, atcmd_output);
		return false;
	}
	...
	if (!chan) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1400),name,command); // Unknown Channel '%s' (usage: %s <#channel_name>)
		clif->message(fd, atcmd_output);
		return false;
	}
	...
	if (ret == HCS_STATUS_ALREADY) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1436),name); // You're already in the '%s' channel
		clif->message(fd, atcmd_output);
		return false;
	}

	if (ret == HCS_STATUS_NOPERM) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1401),name,command); // '%s' Channel is password protected (usage: %s <#channel_name> <password>)
		clif->message(fd, atcmd_output);
		return false;
	}

	if (ret == HCS_STATUS_BANNED) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1438),name); // You cannot join the '%s' channel because you've been banned from it
		clif->message(fd, atcmd_output);
		return false;
	}
...
void atcommand_channel_help(int fd, const char *command, bool can_create) {
	nullpo_retv(command);
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1404),command); // %s failed.
	clif->message(fd, atcmd_output);
	clif->message(fd, msg_txt(1414));// --- Available options:
	if( can_create ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1415),command);// -- %s create <channel name> <channel password>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1416));// - creates a new channel
	}
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1417),command);// -- %s list
	clif->message(fd, atcmd_output);
	clif->message(fd, msg_txt(1418));// - lists public channels
	if( can_create ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1419),command);// -- %s list colors
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1420));// - lists colors available to select for custom channels
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1421),command);// -- %s setcolor <channel name> <color name>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1422));// - changes <channel name> color to <color name>
	}
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1423),command);// -- %s leave <channel name>
	clif->message(fd, atcmd_output);
	clif->message(fd, msg_txt(1424));// - leaves <channel name>
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1427),command);// -- %s bindto <channel name>
	clif->message(fd, atcmd_output);
	clif->message(fd, msg_txt(1428));// - binds global chat to <channel name>, making anything you type in global be sent to the channel
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1429),command);// -- %s unbind
	clif->message(fd, atcmd_output);
	clif->message(fd, msg_txt(1430));// - unbinds your global chat from its attached channel (if binded)
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1429),command);// -- %s unbind
	clif->message(fd, atcmd_output);
	if( can_create ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1456),command);// -- %s ban <channel name> <character name>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1457));// - bans <character name> from <channel name> channel
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1458),command);// -- %s banlist <channel name>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1459));// - lists all banned characters from <channel name> channel
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1460),command);// -- %s unban <channel name> <character name>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1461));// - unbans <character name> from <channel name> channel
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1467),command);// -- %s unbanall <channel name>
		clif->message(fd, atcmd_output);
		clif->message(fd, msg_txt(1468));// - unbans everyone from <channel name>
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1462),command);// -- %s setopt <channel name> <option name> <option value>
		clif->message(fd, atcmd_output);
...
ACMD(channel) {
	...
	if (strcmpi(subcmd,"create") == 0 && (channel->config->allow_user_channel_creation || pc_has_permission(sd, PC_PERM_HCHSYS_ADMIN))) {
		...
		if (sub1[0] != '#') {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1406), HCS_NAME_LENGTH);// Channel length must be between 3 and %d
			clif->message(fd, atcmd_output);
			return false;
		} else if (sub3[0] != '\0') {
		...
		if (strcmpi(sub1 + 1, channel->config->local_name) == 0 || strcmpi(sub1 + 1, channel->config->ally_name) == 0 || strdb_exists(channel->db, sub1 + 1)) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1407), sub1);// Channel '%s' is not available
			clif->message(fd, atcmd_output);
			return false;
		}
		...
		if (sub1[0] != '\0' && strcmpi(sub1,"colors") == 0) {
			for (k = 0; k < channel->config->colors_count; k++) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), "[ %s lista de cores ] : %s", command, channel->config->colors_name[k]);
			...
			if (channel->config->local) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1409), channel->config->local_name, map->list[sd->bl.m].channel ? db_size(map->list[sd->bl.m].channel->users) : 0);// - #%s ( %d users )
				clif->message(fd, atcmd_output);
			}
			if (channel->config->ally && sd->status.guild_id) {
				struct guild *g = sd->guild;
				if( !g ) { dbi_destroy(iter); return false; }
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1409), channel->config->ally_name, db_size(g->channel->users));// - #%s ( %d users )
				clif->message(fd, atcmd_output);
			}
			for (chan = dbi_first(iter); dbi_exists(iter); chan = dbi_next(iter)) {
				if (show_all || chan->type == HCS_TYPE_PUBLIC || chan->type == HCS_TYPE_IRC) {
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1409), chan->name, db_size(chan->users));// - #%s ( %d users )
					clif->message(fd, atcmd_output);
				}
			}
			dbi_destroy(iter);
		...
		if (chan->owner != sd->status.char_id && !pc_has_permission(sd, PC_PERM_HCHSYS_ADMIN)) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1412), sub1);// You're not the owner of channel '%s'
			clif->message(fd, atcmd_output);
			return false;
		}
		...
		if (k == channel->config->colors_count) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1411), sub2);// Unknown color '%s'
			clif->message(fd, atcmd_output);
			return false;
		}
		chan->color = k;
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1413), sub1, channel->config->colors_name[k]);// '%s' channel color updated to '%s'
		clif->message(fd, atcmd_output);
		...
		if (k == sd->channel_count) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1425),sub1);// You're not part of the '%s' channel
			clif->message(fd, atcmd_output);
			return false;
		}
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1426),sub1); // You've left the '%s' channel
		clif->message(fd, atcmd_output);
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1431),sub1); // Your global chat is now bound to the '%s' channel
		clif->message(fd, atcmd_output);
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1433),sd->gcbind->name); // Your global chat is no longer bound to the '#%s' channel
		clif->message(fd, atcmd_output);
		...
		if (!*message || sscanf(message, "%19s %19s %23[^\n]", subcmd, sub1, sub4) < 3) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1434), sub4);// Player '%s' was not found
			clif->message(fd, atcmd_output);
			return false;
		}
		...
		if (ret == HCS_STATUS_ALREADY) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1465), pl_sd->status.name);// Player '%s' is already banned from this channel
			clif->message(fd, atcmd_output);
			return false;
		}
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1437),pl_sd->status.name,sub1); // Player '%s' has now been banned from '%s' channel
		clif->message(fd, atcmd_output);
		...
		if (ret == HCS_STATUS_ALREADY) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1440), pl_sd->status.name);// Player '%s' is not banned from this channel
			clif->message(fd, atcmd_output);
			return false;
		}

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1441),pl_sd->status.name,sub1); // Player '%s' has now been unbanned from the '%s' channel
		clif->message(fd, atcmd_output);
		...
		if (ret == HCS_STATUS_ALREADY) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1439), sub1);// Channel '%s' has no banned players
			clif->message(fd, atcmd_output);
			return false;
		}

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1442),sub1); // Removed all bans from '%s' channel
		clif->message(fd, atcmd_output);
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1443), chan->name);// -- '%s' ban list
		clif->message(fd, atcmd_output);
		...
		for (data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key)) {
			...
			if (!isA)
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1444), entry->name);// - %s %s
			else
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1445), entry->name, key.i);// - %s (%d)
		...
		if (k == 3) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1447), sub2);// '%s' is not a known channel option
			clif->message(fd, atcmd_output);
			clif->message(fd, msg_txt(1448)); // -- Available options
			for (k = 1; k < 3; k++) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1444), opt_str[k]);// - '%s'
				clif->message(fd, atcmd_output);
			}
			return false;
		}
		if (sub3[0] == '\0') {
			if (k == HCS_OPT_MSG_DELAY) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1466), opt_str[k]);// For '%s' you need the amount of seconds (from 0 to 10)
				clif->message(fd, atcmd_output);
				return false;
			} else if (chan->options & k) {
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1449), opt_str[k],opt_str[k]); // option '%s' is already enabled, if you'd like to disable it type '@channel setopt %s 0'
				clif->message(fd, atcmd_output);
				return false;
			} else {
				channel->set_options(chan, chan->options | k);
				safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1450), opt_str[k],chan->name);//option '%s' is now enabled for channel '%s'
				clif->message(fd, atcmd_output);
				return true;
			}
		} else {
			...
			if (k == HCS_OPT_MSG_DELAY) {
				if (v < 0 || v > 10) {
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1451), v, opt_str[k]);// value '%d' for option '%s' is out of range (limit is 0-10)
					clif->message(fd, atcmd_output);
					return false;
				}
				if (v == 0) {
					channel->set_options(chan, chan->options&~k);
					chan->msg_delay = 0;
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1453), opt_str[k],chan->name,v);// option '%s' is now disabled for channel '%s'
					clif->message(fd, atcmd_output);
					return true;
				} else {
					channel->set_options(chan, chan->options | k);
					chan->msg_delay = v;
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1452), opt_str[k],chan->name,v);// option '%s' is now enabled for channel '%s' with %d seconds
					clif->message(fd, atcmd_output);
					return true;
				}
			} else {
				if (v) {
					if (chan->options & k) {
						safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1449), opt_str[k],opt_str[k]); // option '%s' is already enabled, if you'd like to disable it type '@channel opt %s 0'
						clif->message(fd, atcmd_output);
						return false;
					} else {
						channel->set_options(chan, chan->options | k);
					...
					if (!(chan->options & k)) {
						safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1454), opt_str[k],chan->name); // option '%s' is not enabled on channel '%s'
						clif->message(fd, atcmd_output);
						return false;
					} else {
						channel->set_options(chan, chan->options&~k);
						safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1453), opt_str[k],chan->name);// option '%s' is now disabled for channel '%s'
						clif->message(fd, atcmd_output);
						return true;
					}
				}
...
ACMD(fontcolor) {
	...
	if( k == channel->config->colors_count ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1411), message);// Unknown color '%s'
		clif->message(fd, atcmd_output);
		return false;
	}

	sd->fontcolor = k + 1;
	safesnprintf(atcmd_output, sizeof(atcmd_output), "Cor alterada para '%s'", channel->config->colors_name[k]);
	clif->messagecolor_self(fd, channel->config->colors[k], atcmd_output);
...
ACMD(costume){
	...
	if (!*message) {
		for( k = 0; k < len; k++ ) {
			if( sd->sc.data[name2id[k]] ) {
				safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1473),names[k]);//Costume '%s' removed.
				clif->message(sd->fd,atcmd_output);
		...
		for( k = 0; k < len; k++ ) {
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1471),names[k]);//-- %s
			clif->message(sd->fd,atcmd_output);
		}
		return false;
	}
	...
	for( k = 0; k < len; k++ ) {
		if( sd->sc.data[name2id[k]] ) {
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1470),names[k]);// You're already with a '%s' costume, type '@costume' to remove it.
			clif->message(sd->fd,atcmd_output);
			return false;
		}
	}
	...
	if( k == len ) {
		safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1469),message);// '%s' is not a known costume
		clif->message(sd->fd,atcmd_output);
		return false;
	}
...
ACMD(skdebug) {
	safesnprintf(atcmd_output, sizeof(atcmd_output),"segundo: %u; terceiro: %u", sd->sktree.second, sd->sktree.third);
	clif->message(fd,atcmd_output);
...
ACMD(cddebug) {
	...
	if (!(cd = idb_get(skill->cd_db,sd->status.char_id))) {
		clif->message(fd,"Nao foi encontrado lista de cooldown.");
	} else {
		...
		for(i = 0; i < cd->cursor; i++) {
			if( cd->entry[i] ) {
				...
				if( !td || td->func != skill->blockpc_end ) {
					clif->messages(fd,"Encontrada entrada invalida no slot %d para a habilidade %s",i,skill->dbs->db[cd->entry[i]->skidx].name);
	...
	if (!cd || (*message && !strcmpi(message,"reset"))) {
		for(i = 0; i < MAX_SKILL; i++) {
			if( sd->blockskill[i] ) {
				clif->messages(fd,"Habilidade encontrada '%s', desbloqueando...",skill->dbs->db[i].name);
...
void atcommand_basecommands(void) {
	...
	for( i = 0; i < ARRAYLENGTH(atcommand_base); i++ ) {
		if(!atcommand->add(atcommand_base[i].command,atcommand_base[i].func,false)) { // Should not happen if atcommand_base[] array is OK
			ShowDebug("atcommand_basecommands: ACMD_DEF duplicado para '%s'.\n", atcommand_base[i].command);
			continue;
		}
	}
		...
		for(i = 0; i < map->list[sd->bl.m].zone->disabled_commands_count; i++) {
			if( info->func == map->list[sd->bl.m].zone->disabled_commands[i]->cmd ) {
				if (pc_get_group_level(sd) < map->list[sd->bl.m].zone->disabled_commands[i]->group_lv) {
					clif->messagecolor_self(sd->fd, COLOR_RED, "Esse comando esta desabilitado nessa area.");
					return true;
				} else {
...
void atcommand_config_read(const char* config_filename) {
	...
	if (aliases != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			if ( !( commandinfo = atcommand->exists(commandname) ) ) {
				ShowConfigWarning(command, "atcommand_config_read: nao se pode definir pseudonimo para comando nao-existente %s", commandname);
				continue;
			}
			...
			for (j = 0; j < alias_count; ++j) {
				...
				if (alias != NULL) {
					...
					if (strdb_exists(atcommand->alias_db, alias)) {
						ShowConfigWarning(command, "atcommand_config_read: pseudonimo %s ja existe.", alias);
						continue;
					}
	...
	if (nolog != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			commandname = config_setting_name(command);
			if ( !( commandinfo = atcommand->exists(commandname) ) ) {
				ShowConfigWarning(command, "atcommand_config_read: nao se pode desabilitar logging para comando nao-existente %s", commandname);
				continue;
			}
...
void atcommand_db_load_groups(GroupSettings **groups, config_setting_t **commands_, size_t sz)
{
	...
	for (atcmd = dbi_first(iter); dbi_exists(iter); atcmd = dbi_next(iter)) {
		...
		for (i = 0; i < sz; i++) {
			...
			if (idx < 0 || idx >= sz) {
				ShowError("atcommand_db_load_groups: index (%d) fora dos limites [0,%"PRIuS"]\n", idx, sz - 1);
				continue;
			}
